...
export class StatsService {
  ...
  async getCategoryBreakdown(userId: string, query: StatsQueryDto) {
    ...
    const categoryIds = result.map(r => r.category_id).filter((id): id is string =>Boolean(id));
    ...
    return result.map(r => ({
      ...
      categoryName: r.category_id? (categoryMap.get(r.category_id)?.name  || 'Sans catégorie') : 'Sans Categorie',
      color: r.category_id ?(categoryMap.get(r.category_id)?.icon || '#ccc') :  '#ccc',
  ...
  async getMonthlyTrends(userId: string, year: number) {
    ...
    for (const t of transactions) {
      ...
      if (t.type === TransactionType.income) monthly[month].income += t.amount.toNumber();
      else monthly[month].expense += t.amount.toNumber();
    }
    return monthly.map((m, idx) => ({
        month: idx + 1,
        monthName: new Date(year, idx, 1).toLocaleString('default', { month: 'short' }),
        income: m.income,
        expense: m.expense,
    ...
    async getDailySpending(userId: string, query: StatsQueryDto) {
      const { startDate, endDate } = this.parseDateRange(query);
      if (!startDate || !endDate) {
        throw new BadRequestException('Pour le détail journalier, fournissez startDate/endDate ou period');
      }
      ...
      for (const t of transactions) {
        ...
        if (entry) {
          if (t.type === TransactionType.income) entry.income += t.amount.toNumber();
